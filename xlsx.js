		...
		var write_num = (function make_write_num() {
			...
			function write_num_flt(type, fmt, val) {
				...
				if ((r = fmt.match(/^00,000\.([#0]*0)$/))) {
					...
					return val < 0
						? '-' + write_num_flt(type, fmt, -val)
						: commaify(flr(val)).replace(/^\d,\d{3}$/, '0$&').replace(/^\d*$/, function($$) {
			...
			function write_num_int(type, fmt, val) {
				...
				if ((r = fmt.match(/^00,000\.([#0]*0)$/))) {
					return val < 0
						? '-' + write_num_int(type, fmt, -val)
						: commaify('' + val).replace(/^\d,\d{3}$/, '0$&').replace(/^\d*$/, function($$) {
								return '00,' + ($$.length < 3 ? pad0(0, 3 - $$.length) : '') + $$;
							}) +
							'.' +
							pad0(0, r[1].length);
...
*/
	...
	function parse_PropertySet(blob, PIDSI) {
		...
		for (i = 0; i != NumProps; ++i) {
			...
			if (PIDSI) {
				...
				if (piddsi.p === 'version')
					PropH[piddsi.n] =
						String(PropH[piddsi.n] >> 16) + '.' + ('0000' + String(PropH[piddsi.n] & 0xffff)).slice(-4);
	...
	*/
	...
	/* 18.3 Worksheets */
	function parse_ws_xml(data, opts, idx, rels, wb) {
		if (!data) return data;
	...
	function parse_ws_bin(data, _opts, idx, rels, wb, themes, styles) {
		...
		if (
			!s['!ref'] &&
			(refguess.s.r < 2000000 || (ref && (ref.e.r > 0 || ref.e.c > 0 || ref.s.r > 0 || ref.s.c > 0)))
		)
	...
	function write_BUNDLESHS(ba, wb) {
		...
		for (var idx = 0; idx != wb.SheetNames.length; ++idx) {
			var viz =
				(wb.Workbook && wb.Workbook.Sheets && wb.Workbook.Sheets[idx] && wb.Workbook.Sheets[idx].Hidden) || 0;
	...
	function parse_ws(data, name, idx, opts, rels, wb, themes, styles) {
		...
		return parse_ws_xml(data, opts, idx, rels, wb);
	}
	...
	function write_ws_xlml_wsopts(ws, opts, idx, wb) {
		...
		if (wb && wb.Workbook && wb.Workbook.Sheets && wb.Workbook.Sheets[idx]) {
			...
			if (wb.Workbook.Sheets[idx].Hidden)
				o.push(
					writextag('Visible', wb.Workbook.Sheets[idx].Hidden == 1 ? 'SheetHidden' : 'SheetVeryHidden', {})
				);
			else {
				/* Selected */
...
*/
		...
		if (wb.Props) {
			...
			for (i = 0; i < Keys.length; ++i)
				(DocSummaryRE.hasOwnProperty(Keys[i])
					? DSEntries
					: SummaryRE.hasOwnProperty(Keys[i]) ? SEntries : CEntries).push([ Keys[i], wb.Props[Keys[i]] ]);
		}
		if (wb.Custprops) {
			Keys = keys(wb.Custprops);
			// $FlowIgnore
			for (i = 0; i < Keys.length; ++i)
				if (!(wb.Props || {}).hasOwnProperty(Keys[i]))
					(DocSummaryRE.hasOwnProperty(Keys[i])
						? DSEntries
						: SummaryRE.hasOwnProperty(Keys[i]) ? SEntries : CEntries).push([
						Keys[i],
						wb.Custprops[Keys[i]]
					]);
		}
	...
	function safe_parse_sheet(zip, path, relsPath, sheet, idx, sheetRels, sheets, stype, opts, wb) {
		try {
			...
			switch (stype) {
				case 'sheet':
					_ws = parse_ws(data, path, idx, opts, sheetRels[sheet], wb);
					break;
				case 'chart':
	...
	function parse_zip(zip, opts) {
		...
		var wbrelsfile = (dir.workbooks[0].slice(0, wbrelsi + 1) +
			'_rels/' +
